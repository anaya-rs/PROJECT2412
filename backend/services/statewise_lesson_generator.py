"""
State-wise Lesson Generator with integrated question generation
"""
...
class StatewiseLessonGenerator:
    """Main state-wise lesson generator with clean architecture"""
    ...
    def __init__(self, ollama_url: str, model: str, db_session):
        ...
        logger.info(f"Initialized StatewiseLessonGenerator with model: {model}")
    ...
    def generate_lesson(
        ...
        try:
            logger.info(f"Starting state-wise lesson generation: duration={duration_minutes}")
    ...
    def _generate_raw_question(self, source_content: str, spec: QuestionSpec, 
        ...
        """Generate raw question using LLM"""
...
"""
    ...
    def _generate_end_notes_state(self, state_number: int, total_states: int, source_content: str, previous_context: str) -> EndNotesState:
        ...
        if len(takeaways) < 3:
            # Add generic takeaways if LLM didn't provide enough
            generic_takeaways = [
    ...
    def _call_llm(self, prompt: str) -> str:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                raise StatewiseGenerationError(f"LLM call failed: {response.status_code}")
                
        except requests.exceptions.Timeout:
            raise StatewiseGenerationError("LLM call timeout")
        except Exception as e:
            raise StatewiseGenerationError(f"LLM call error: {e}")
    ...
    def _parse_json_response(self, response: str) -> Dict[str, Any]:
        ...
        try:
            ...
            raise StatewiseGenerationError(f"Invalid JSON in LLM response: {e}")
