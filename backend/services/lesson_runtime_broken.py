...
class LessonRuntimeService:
    ...
    def _generate_contextual_hint(self, question_prompt: str, options: list, correct_answers: list, lesson_context: str) -> str:
        """Generate a meaningful contextual hint based on question and lesson content"""
        
        # Extract key concepts from the question
        question_lower = question_prompt.lower()
        
        # Python-specific hints
        if "python" in question_lower:
            if "variable" in question_lower:
                return "Hint: Think about how variables store data and what makes them valid in Python."
            elif "function" in question_lower:
                return "Hint: Consider the key components of a Python function - what does it need to work properly?"
            elif "list" in question_lower or "array" in question_lower:
                return "Hint: Remember how Python lists are structured and what operations you can perform on them."
            elif "loop" in question_lower:
                return "Hint: Think about the purpose of loops and how they control program flow."
            elif "conditional" in question_lower or "if" in question_lower:
                return "Hint: Consider how conditional statements check conditions and make decisions."
        
        # General programming hints
        if "syntax" in question_lower:
            return "Hint: Pay attention to the exact syntax rules - programming languages are very specific!"
        elif "error" in question_lower:
            return "Hint: Think about what causes common errors and how to prevent them."
        elif "output" in question_lower or "print" in question_lower:
            return "Hint: Consider how programs display information and what methods are available."
        
        # Use lesson context for more specific hints
        if lesson_context:
            # Look for key terms in the lesson content
            context_lower = lesson_context.lower()
            if "data type" in context_lower:
                return "Hint: Consider the different data types you've learned about and their characteristics."
            elif "syntax" in context_lower:
                return "Hint: Review the syntax rules covered in the lesson material."
            elif "example" in context_lower:
                return "Hint: Think about the examples you've seen and what patterns they follow."
        
        # Fallback to question-specific hints
        if correct_answers and options:
            correct_idx = correct_answers[0] if correct_answers else 0
            if 0 <= correct_idx < len(options):
                correct_option = options[correct_idx]
                # Provide a hint about the concept without giving away the answer
                if "definition" in question_lower:
                    return f"Hint: Focus on the precise meaning and purpose of the concept being asked about."
                elif "purpose" in question_lower or "why" in question_lower:
                    return f"Hint: Think about the practical applications and benefits."
                else:
                    return f"Hint: Consider what makes the most logical sense based on what you've learned."
        
        return "Hint: Review the lesson material and think about the key concepts covered."
    ...
    def submit_action(self, session_id: str, user_id: int, action) -> Dict[str, Any]:
        ...
        if action_type == "next":
            logger.info(f"🔍 [NEXT DEBUG] Processing next action for session {session_id}")
            logger.info(f"🔍 [NEXT DEBUG] Current state type: {current_state.get('type')}")
            
            # Allow next from content states
            if current_state.get("type") == "question":
                logger.info(f"🔍 [NEXT DEBUG] In question state, checking answer status")
                ...
                last_answer_status = session_runtime.session_data.get("last_answer_status")
                logger.info(f"🔍 [NEXT DEBUG] Last answer status: {last_answer_status}")
                ...
                if last_answer_status not in ["correct", "reveal_answer"]:
                    error_msg = "Cannot advance from question state without correct answer or exhausted attempts"
                    logger.error(f"🔍 [NEXT DEBUG] {error_msg}")
                    raise ValueError(error_msg)
                else:
                    logger.info(f"🔍 [NEXT DEBUG] Allowing next after {last_answer_status} for session {session_id}")
            else:
                logger.info(f"🔍 [NEXT DEBUG] In content state, allowing next")
            
            # Move to next state
            logger.info(f"🔍 [NEXT DEBUG] Moving from index {session_runtime.current_index} to {session_runtime.current_index + 1}")
            ...
            # Commit the changes
            logger.info(f"🔍 [NEXT DEBUG] Committing changes to database")
            self.db.commit()
            
            # Return the next state
            next_index = session_runtime.current_index
            logger.info(f"🔍 [NEXT DEBUG] Next index: {next_index}, total states: {len(states_data)}")
            
            if next_index >= len(states_data):
                logger.info(f"🔍 [NEXT DEBUG] Lesson completed")
                return {
                    "state": None,
                    "progress": 1.0,
                    "attempts_left": 0,
                ...
                next_state = states_data[next_index]
                logger.info(f"🔍 [NEXT DEBUG] Returning next state: {next_state.get('type')}")
                return {
                    "state": next_state,
                    "progress": next_index / len(states_data),
                    "attempts_left": 3,
                    "completed": False,
                    "status": None
                }
        ...
        if action_type == "next":
            ...
            # CRITICAL DEBUG: Log of raw values before any conversion
            ...
            # Handle hint request
            if current_state.get("type") != "question":
                raise ValueError("Cannot request hint for non-question state")
            
            # Generate meaningful contextual hint
            try:
                import json
                
                # Get lesson context for better hint generation
                lesson = self.db.query(LessonDB).filter(LessonDB.id == session_runtime.lesson_id).first()
                
                # Prepare hint generation data
                question_prompt = current_state.get("prompt", "")
                options = current_state.get("options", [])
                correct_answers = current_state.get("correct_answers", [])
                
                # Get lesson context safely
                lesson_context = ""
                if lesson and lesson.states:
                    try:
                        states_data = json.loads(lesson.states)
                        # Get first few content states for context
                        content_states = [state for state in states_data if state.get("type") == "content"]
                        if content_states:
                            lesson_context = " ".join([state.get("text", "") for state in content_states[:2]])
                    except (json.JSONDecodeError, AttributeError):
                        lesson_context = ""
                
                # Generate contextual hint based on question and lesson content
                hint = self._generate_contextual_hint(question_prompt, options, correct_answers, lesson_context)
                
            except Exception as e:
                logger.warning(f"Failed to generate contextual hint: {e}")
                # Simple fallback
                hint = "Hint: Think carefully about the question and consider what you've learned in the lesson."
            ...
            # Handle skip request
            if current_state.get("type") != "question":
                raise ValueError("Cannot skip non-question state")
            
            # Store status in session data for next action validation
            if not session_runtime.session_data:
                session_runtime.session_data = {}
            
            session_runtime.session_data["last_answer_status"] = "skipped"
            session_runtime.attempts = 0  # Reset attempts for next state
            session_runtime.current_index += 1  # Move to next state
            session_runtime.last_active_at = datetime.utcnow()
